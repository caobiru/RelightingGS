...
def calc_score(inputs, images, extrinsic, points3d, args):
    i, j = inputs
    ...
    for pid in id_intersect:
        ...
        theta = (180 / np.pi) * np.arccos(
            np.dot(cam_center_i - p, cam_center_j - p) / np.linalg.norm(cam_center_i - p) / np.linalg.norm(cam_center_j - p)
        )
        score += np.exp(
            -(theta - args.theta0) ** 2 / (2 * (args.sigma1 if theta <= args.theta0 else args.sigma2) ** 2)
        )
...
if __name__ == '__main__':
    ...
    with mp.Pool(processes=mp.cpu_count()) as p:
        result = p.starmap(calc_score, [(inputs, images, extrinsic, points3d, args) for inputs in queue])
