    ...
    for i, sample in pbar:
        ...
        if (i % 49 == 0 or True) and (args.show_result or args.write_result):
            ...
            if args.write_result:
                ...
                print(f"Saving outputs for sample: {sample_name} to {args.result_dir}")

                # Save reference image
                output_img_path = os.path.join(args.result_dir, f'{sample_name}.jpg')
                cv2.imwrite(output_img_path, ref_o)
                print(f"Saved reference image: {output_img_path}")

                # Save camera file
                output_cam_path = os.path.join(args.result_dir, f'cam_{sample_name}_flow3.txt')
                write_cam(output_cam_path, ref_cam_o)
                print(f"Saved camera file: {output_cam_path}")

                # Save depth map
                output_depth_path = os.path.join(args.result_dir, f'{sample_name}_flow3.pfm')
                write_pfm(output_depth_path, est_depth_o)
                print(f"Saved depth map: {output_depth_path}")

                # Save probability maps
                for stage_i, prob_map_o in enumerate(prob_maps_o):
                    output_prob_path = os.path.join(args.result_dir, f'{sample_name}_flow{stage_i+1}_prob.pfm')
                    write_pfm(output_prob_path, prob_map_o)
                    print(f"Saved probability map: {output_prob_path}")
